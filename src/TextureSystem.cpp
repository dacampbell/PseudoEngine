...
void TextureSystem::initSystem() {
	this->viewport = new ViewPort(Engine::camera->getWidth(), Engine::camera->getHeight(), Engine::world->getTextureManager());
}
	...
	for(int l = 0; l < 32; l++) {
		for(auto item : *(this->getComponents())) {
			...
			if(locX > camX-32 && locX < camX + camW &&
				locY > camY-32 && locY < camY + camH && layer == l) {
